...
class SpectralConv2dV2(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        out = torch.fft.irfft2(out_ft, dim=(-2,-1), norm='ortho', s=(size_0, size_1)).to(dtype)
        return out
            # xlst.append(torch.fft.irfft2(out_ft, dim=(-2,-1), norm='ortho', s=(size_0, size_1)).to(dtype))
        

        #Return to physical space
        return torch.cat(xlst, dim=0).to(device)
